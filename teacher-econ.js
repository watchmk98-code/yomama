...
(function(){
  ...
  var teacher;try{teacher=JSON.parse(localStorage.getItem('yomama_teacher_v1')||'null');}catch(_){}
  if(!teacher || !teacher.teacher_token){root.innerHTML='<h2>LIVE CLASS ECONOMY</h2><p><a href="./class.html">Open or resume your class</a> to view students and publish a market event.</p>';return;}
    ...
    return fetch('/api/game/teacher/econ?teacher_token='+encodeURIComponent(teacher.teacher_token)).then(function(r){return r.json().then(function(d){if(!r.ok)throw Error(d.error);return d;});}).then(function(d){
      ...
      if(!document.getElementById('econ-students')){
        ...
        form.addEventListener('submit',function(e){
          e.preventDefault();var submit=form.querySelector('button');submit.disabled=true;
  ...
  refresh();setInterval(function(){if(!document.hidden)refresh();},15000);
}());
