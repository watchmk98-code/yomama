...
(function(){
  ...
  function compact(){return innerWidth<900 || innerHeight<=650;}
  ...
  function sectionMenu(){
    ...
    Array.from(nav.querySelectorAll('a:not([hidden])')).forEach(function(a){var o=document.createElement('option');o.value=a.getAttribute('href');o.textContent=a.textContent;o.selected=a.getAttribute('href')==='./buildings.html';select.appendChild(o);});
  }
  function tabset(root,key,items){
    ...
    items.forEach(function(i){i[1].classList.remove('game-view-hidden');});
    if(!compact())return;
    ...
    items.forEach(function(item,index){
      var button=document.createElement('button');button.type='button';button.textContent=item[0];button.setAttribute('role','tab');button.setAttribute('aria-selected',String(index===selected));button.tabIndex=index===selected?0:-1;
      item[1].id=item[1].id || 'game-view-'+key+'-'+index;button.setAttribute('aria-controls',item[1].id);
      button.addEventListener('click',function(){choose(key,index);root.querySelectorAll('.game-view-tabs button')[index].focus();});
      button.addEventListener('keydown',function(e){if(e.key==='ArrowRight'||e.key==='ArrowLeft'){e.preventDefault();var next=(index+(e.key==='ArrowRight'?1:-1)+items.length)%items.length;choose(key,next);root.querySelectorAll('.game-view-tabs button')[next].focus();}});
  ...
  function render(){
    ...
    if(build){
      tabset(build,'Build',[['Building',build.querySelector('.game-site')],['Goods',build.querySelector('.game-stock')],['Upgrades',build.querySelector('.game-operations')],['Expand',build.querySelector('.game-expansion')]]);
      ...
      if(compact() && window.YomamaEcon){var s=window.YomamaEcon.state();if(s){picker=document.createElement('select');picker.className='game-fit-picker';picker.setAttribute('aria-label','Selected building');s.buildings.forEach(function(b){var o=document.createElement('option');o.value=b.slot;o.textContent=b.name;o.selected=!!build.querySelector('[data-select-building="'+b.slot+'"][aria-pressed="true"]');picker.appendChild(o);});picker.onchange=function(){var b=build.querySelector('[data-select-building="'+picker.value+'"]');if(b)b.click();};build.insertBefore(picker,build.firstChild);}}
    }
    if(warehouse)tabset(warehouse,'Warehouse',[['Stock',warehouse.querySelector('.game-inventory-table')],['Storage',warehouse.querySelector('.game-purpose-side')]]);
    if(market){tabset(market,'Market',[['Orders',market.querySelector('.game-market-orders')],['Customers',market.querySelector('.game-market-tools > section:first-child')],['Surplus',market.querySelector('.game-market-tools > section:last-child')]]);var pick=market.querySelector('.game-business-picker');if(pick)pick.classList.toggle('game-view-hidden',compact() && !(tabs.Market||0));}
    if(ops)tabset(ops,'Operations',[['Chains',ops.querySelector('.game-purpose-main')],['Upgrade',ops.querySelector('.game-purpose-side')]]);
    ...
    paginate('.game-market-orders .game-order-grid','Orders',compact()?10000:160,compact()?1:3);
