...
## Business assets
45 new icons in `business-assets-01.png` through `business-assets-03.png`, each a 3-column × 5-row atlas. Generated with the built-in image tool; prompts and ordered asset IDs are in `business-assets-prompts.json`. Original PNG pixels are preserved; `craft-art.js` contains measured display rectangles. Teal/violet borders are CSS, not baked into the art.
