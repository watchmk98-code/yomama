(() => {
  ...
  const formatCashLabel = (value) => `$${Math.round(value).toLocaleString('en-US')}`;
  const gameUiUsername = gameAccountUsername.toLowerCase();

  const renderGlobalCashChip = () => {
    const rightMeta = document.querySelector('.right-meta');
    if (!rightMeta) return;

    let cashChip = document.getElementById('game-cash-chip');
    if (!cashChip) {
      cashChip = document.createElement('span');
      cashChip.id = 'game-cash-chip';
      cashChip.className = 'chip amber';
      rightMeta.insertBefore(cashChip, rightMeta.firstChild);
    }

    const cash = Math.max(0, toSafeNumber(gameSaveState?.wallet?.cash, 0));
    cashChip.textContent = `USER ${gameUiUsername.toUpperCase()} | CASH ${formatCashLabel(cash)}`;
  };

  renderGlobalCashChip();
  ...
  const getBuildingUpgradeCost = (building) => {
    const config = getBuildingCatalogEntry(building.type);
    const safeLevel = Math.max(1, toPositiveInteger(building.level, 1));
    return Math.round(config.upgradeBaseCost * Math.pow(config.upgradeCostGrowth, safeLevel - 1));
  };

  const createBuildingInstance = (type, now = Date.now()) => {
    const config = getBuildingCatalogEntry(type);
    return {
      id: `bld_${buildRandomHex(12)}`,
      type: config.key,
      level: 1,
      createdAt: now,
      lastCalcAt: now,
      ...
      totalCollected: 0,
      upgradeEndsAt: 0,
      costBasis: { cashSpent: config.purchaseCost },
    };
  };
  ...
  if (buildingsLabRoot) {
    ...
    const statKeys = Object.freeze(['AGR', 'FAB', 'PWR', 'COM', 'LOG']);
    const resourceKeys = Object.freeze(['food', 'materials', 'energy', 'data']);
    const baseInventoryCaps = Object.freeze({
      food: 1500,
      materials: 1200,
      energy: 1000,
      data: 700,
    ...
    const charactersByBio = Object.freeze({
      ...
      irene: 'defensive growth drawdown control capital preservation',
      derdo: 'high-beta narrative momentum sentiment inflection liquidity',
      ozan: 'industrial gambler volatility bitcoin factory',
      hussein: 'silent operator factory off-radar',
      can: 'real sector production machinery infrastructure operator',
      hara: 'private art markets scarcity narrative asymmetry',
      pelli: 'signal hunter quant strategist datasets structural shifts',
    });
    const classDefinitions = Object.freeze({
      homesteader: {
        id: 'homesteader',
        name: 'Homesteader',
        startStats: { AGR: 8, FAB: 2, PWR: 2, COM: 2, LOG: 4 },
        traitName: 'Harvest Surge',
        traitDescription: '1/day: arm next Farm collection for +20% Food.',
        keywords: ['farm', 'agronomy', 'harvest', 'field', 'rural', 'soil'],
      },
      factory_foreman: {
        id: 'factory_foreman',
        name: 'Factory Foreman',
        startStats: { AGR: 2, FAB: 8, PWR: 2, COM: 2, LOG: 4 },
        traitName: 'Standardize Parts',
        traitDescription: 'Workshop upgrades use -5% Materials after normal multiplier.',
        keywords: ['factory', 'industrial', 'production', 'machinery', 'parts'],
      },
      grid_engineer: {
        id: 'grid_engineer',
        name: 'Grid Engineer',
        startStats: { AGR: 2, FAB: 2, PWR: 8, COM: 2, LOG: 4 },
        traitName: 'Stable Output',
        traitDescription: 'Generator offline cap +60 minutes (pre-automation cap still applies).',
        keywords: ['grid', 'power', 'energy', 'infrastructure', 'systems'],
      },
      data_architect: {
        id: 'data_architect',
        name: 'Data Architect',
        startStats: { AGR: 2, FAB: 2, PWR: 2, COM: 8, LOG: 4 },
        traitName: 'Clean Pipelines',
        traitDescription: 'Data Center +10% output if collected within 10m after cycle boundary.',
        keywords: ['data', 'quant', 'signal', 'pipeline', 'comput', 'analysis'],
      },
      logistics_chief: {
        id: 'logistics_chief',
        name: 'Logistics Chief',
        startStats: { AGR: 3, FAB: 3, PWR: 3, COM: 3, LOG: 6 },
        traitName: 'Queue Order',
        traitDescription: 'Optional queue expansion is deferred in v0.1.',
        keywords: ['logistics', 'cross-border', 'flow', 'supply', 'route', 'dispatch'],
      },
      administrator: {
        id: 'administrator',
        name: 'Administrator',
        startStats: { AGR: 4, FAB: 4, PWR: 4, COM: 4, LOG: 2 },
        traitName: 'Balanced Budget',
        traitDescription: '1/day: first build/upgrade gets -5% all resources.',
        keywords: ['operator', 'allocation', 'discipline', 'balance', 'capital'],
      },
    });
    const classOrder = Object.freeze(Object.keys(classDefinitions));
    const resolvedLabView = String(buildingsLabRoot.getAttribute('data-buildings-view') || '').trim().toLowerCase();
    const buildingsLabView = resolvedLabView === 'advanced-hq'
      ? 'advanced-hq'
      : (resolvedLabView === 'collect' ? 'collect' : 'full');
    const showToolbar = buildingsLabView === 'full';
    const showResourcesSection = buildingsLabView === 'full' || buildingsLabView === 'collect';
    const showProductionSection = buildingsLabView === 'full' || buildingsLabView === 'collect';
    const showAdvancedSection = buildingsLabView === 'full' || buildingsLabView === 'advanced-hq';
    const showSidePanel = true;
    if (buildingsSidePanel) buildingsSidePanel.hidden = !showSidePanel;
    ...
    const getBerlinDayKey = (now) => berlinDayFormatter.format(new Date(now));
    const getBerlinTimeLabel = (now) => berlinTimeFormatter.format(new Date(now));
    const formatNumber = (value) => Math.floor(Math.max(0, Number(value) || 0)).toLocaleString('en-US');
    ...
    const normalizeStatBlock = (value, fallback) => {
      const source = value && typeof value === 'object' ? value : {};
      const base = fallback && typeof fallback === 'object'
        ? fallback
        : { AGR: 0, FAB: 0, PWR: 0, COM: 0, LOG: 0 };
      const next = {};
      statKeys.forEach((key) => {
        next[key] = clamp(toSafeInteger(source[key], base[key] || 0), 0, 10);
      });
      return next;
    };

    const buildFallbackEconomyConfig = () => ({
      meta: { ruleset: 'v0.1', timezone: 'Europe/Berlin' },
      resources: {
        food: { label: 'Food', baseCap: baseInventoryCaps.food },
        materials: { label: 'Materials', baseCap: baseInventoryCaps.materials },
        energy: { label: 'Energy', baseCap: baseInventoryCaps.energy },
        data: { label: 'Data', baseCap: baseInventoryCaps.data },
      },
      productionBuildings: {
        farm: {
          name: 'Greenfield Farm',
          sampleFeatures: ['Irrigation Kit', 'Soil Scanner'],
          sampleStaff: ['Field Lead Arda', 'Harvester Mina'],
          resource: 'food',
          primaryStat: 'AGR',
          cycleMinutes: 120,
          maxLevel: 8,
          baseOutputPerCycle: [30, 45, 64, 88, 118, 154, 198, 250],
          buildCost: [{ food: 40, materials: 30, energy: 10, data: 0 }],
          upgradeCost: [
            { food: 85, materials: 55, energy: 18, data: 0 },
            { food: 130, materials: 84, energy: 28, data: 6 },
            { food: 190, materials: 122, energy: 42, data: 10 },
            { food: 272, materials: 174, energy: 60, data: 16 },
            { food: 378, materials: 242, energy: 84, data: 24 },
            { food: 510, materials: 326, energy: 112, data: 34 },
            { food: 675, materials: 432, energy: 148, data: 46 },
          ],
          buildSeconds: [120],
          upgradeSeconds: [180, 240, 330, 420, 520, 640, 780],
        },
        workshop: {
          name: 'Ironworks Shop',
          sampleFeatures: ['CNC Line', 'Parts QA'],
          sampleStaff: ['Foreman Kaya', 'Assembler Vera'],
          resource: 'materials',
          primaryStat: 'FAB',
          cycleMinutes: 240,
          maxLevel: 8,
          baseOutputPerCycle: [18, 28, 41, 58, 79, 104, 134, 170],
          buildCost: [{ food: 28, materials: 40, energy: 20, data: 4 }],
          upgradeCost: [
            { food: 52, materials: 88, energy: 28, data: 8 },
            { food: 80, materials: 136, energy: 42, data: 12 },
            { food: 116, materials: 196, energy: 60, data: 18 },
            { food: 166, materials: 278, energy: 84, data: 26 },
            { food: 230, materials: 384, energy: 116, data: 36 },
            { food: 308, materials: 514, energy: 154, data: 48 },
            { food: 410, materials: 676, energy: 202, data: 64 },
          ],
          buildSeconds: [150],
          upgradeSeconds: [210, 300, 390, 480, 600, 740, 900],
        },
        generator: {
          name: 'North Grid Plant',
          sampleFeatures: ['Load Balancer', 'Turbine Shield'],
          sampleStaff: ['Grid Tech Emre', 'Operator Lina'],
          resource: 'energy',
          primaryStat: 'PWR',
          cycleMinutes: 240,
          maxLevel: 8,
          baseOutputPerCycle: [16, 24, 35, 50, 69, 92, 120, 154],
          buildCost: [{ food: 24, materials: 30, energy: 24, data: 8 }],
          upgradeCost: [
            { food: 44, materials: 56, energy: 70, data: 12 },
            { food: 68, materials: 84, energy: 106, data: 18 },
            { food: 98, materials: 120, energy: 154, data: 26 },
            { food: 140, materials: 170, energy: 220, data: 36 },
            { food: 194, materials: 236, energy: 304, data: 48 },
            { food: 262, materials: 320, energy: 404, data: 64 },
            { food: 350, materials: 428, energy: 536, data: 82 },
          ],
          buildSeconds: [160],
          upgradeSeconds: [220, 300, 390, 500, 620, 760, 920],
        },
        data_center: {
          name: 'Node-7 Data Hub',
          sampleFeatures: ['Hot/Cold Aisle', 'Clean Pipeline'],
          sampleStaff: ['Sys Architect Ivo', 'Ops Analyst Sara'],
          resource: 'data',
          primaryStat: 'COM',
          cycleMinutes: 360,
          maxLevel: 8,
          baseOutputPerCycle: [12, 18, 27, 38, 52, 70, 92, 118],
          buildCost: [{ food: 18, materials: 28, energy: 34, data: 20 }],
          upgradeCost: [
            { food: 38, materials: 50, energy: 62, data: 76 },
            { food: 56, materials: 74, energy: 92, data: 114 },
            { food: 82, materials: 106, energy: 134, data: 166 },
            { food: 116, materials: 150, energy: 192, data: 236 },
            { food: 160, materials: 208, energy: 264, data: 326 },
            { food: 214, materials: 282, energy: 354, data: 438 },
            { food: 286, materials: 376, energy: 468, data: 578 },
          ],
          buildSeconds: [210],
          upgradeSeconds: [260, 350, 460, 590, 730, 880, 1060],
        },
      },
      advancedBuildings: {
        hq: {
          name: 'Skyline HQ',
          sampleFeatures: ['Command Bridge', 'Executive Ops'],
          sampleStaff: ['Chief Ada', 'Ops Chief Leo'],
          maxLevel: 5,
          buildCost: [{ food: 0, materials: 0, energy: 0, data: 0 }],
          upgradeCost: [
            { food: 140, materials: 180, energy: 120, data: 85 },
            { food: 220, materials: 282, energy: 184, data: 132 },
            { food: 324, materials: 408, energy: 268, data: 194 },
            { food: 460, materials: 570, energy: 374, data: 272 },
          ],
          buildSeconds: [0],
          upgradeSeconds: [300, 420, 560, 740],
        },
        warehouse: {
          name: 'Atlas Warehouse',
          sampleFeatures: ['Racked Storage', 'Cold Chamber'],
          sampleStaff: ['Storemaster Yara', 'Forklift Ken'],
          maxLevel: 6,
          buildCost: [{ food: 0, materials: 0, energy: 0, data: 0 }],
          upgradeCost: [
            { food: 80, materials: 124, energy: 54, data: 28 },
            { food: 122, materials: 188, energy: 80, data: 42 },
            { food: 176, materials: 268, energy: 114, data: 58 },
            { food: 248, materials: 370, energy: 158, data: 80 },
            { food: 334, materials: 500, energy: 214, data: 108 },
          ],
          buildSeconds: [0],
          upgradeSeconds: [210, 300, 390, 480, 590],
        },
        logistics: {
          name: 'Mercury Logistics',
          sampleFeatures: ['Route Matrix', 'Dispatch Radar'],
          sampleStaff: ['Route Chief Nia', 'Dispatcher Omer'],
          maxLevel: 5,
          buildCost: [{ food: 90, materials: 110, energy: 70, data: 44 }],
          upgradeCost: [
            { food: 126, materials: 156, energy: 98, data: 62 },
            { food: 176, materials: 216, energy: 136, data: 86 },
            { food: 242, materials: 292, energy: 184, data: 116 },
            { food: 324, materials: 388, energy: 246, data: 154 },
          ],
          buildSeconds: [180],
          upgradeSeconds: [240, 320, 420, 540],
        },
        marketplace: {
          name: 'Bazaar Exchange',
          sampleFeatures: ['Price Board', 'Auction Desk'],
          sampleStaff: ['Broker Talia', 'Market Clerk Yusuf'],
          maxLevel: 5,
          buildCost: [{ food: 78, materials: 96, energy: 60, data: 40 }],
          upgradeCost: [
            { food: 112, materials: 136, energy: 84, data: 56 },
            { food: 154, materials: 188, energy: 116, data: 78 },
            { food: 208, materials: 256, energy: 156, data: 104 },
            { food: 278, materials: 344, energy: 208, data: 138 },
          ],
          buildSeconds: [170],
          upgradeSeconds: [220, 300, 390, 500],
        },
        contractor_office: {
          name: 'Contractor Desk',
          sampleFeatures: ['Crew Planner', 'Permit Stack'],
          sampleStaff: ['Builder Rina', 'Planner Murat'],
          maxLevel: 5,
          buildCost: [{ food: 88, materials: 120, energy: 72, data: 36 }],
          upgradeCost: [
            { food: 128, materials: 176, energy: 102, data: 52 },
            { food: 180, materials: 246, energy: 142, data: 72 },
            { food: 246, materials: 336, energy: 192, data: 98 },
            { food: 332, materials: 452, energy: 258, data: 130 },
          ],
          buildSeconds: [210],
          upgradeSeconds: [280, 370, 480, 620],
        },
        automation: {
          name: 'Auto Control',
          sampleFeatures: ['Cycle Scripts', 'Failsafe Watcher'],
          sampleStaff: ['Automation Eng. Ipek', 'Control Op Nox'],
          maxLevel: 5,
          buildCost: [{ food: 102, materials: 132, energy: 94, data: 86 }],
          upgradeCost: [
            { food: 146, materials: 188, energy: 132, data: 122 },
            { food: 202, materials: 258, energy: 182, data: 170 },
            { food: 276, materials: 350, energy: 244, data: 232 },
            { food: 364, materials: 462, energy: 322, data: 306 },
          ],
          buildSeconds: [260],
          upgradeSeconds: [330, 430, 560, 710],
        },
      },
      automation: {
        extendedCapMinutesBase: 180,
        extendedCapMinutesPerLevel: 120,
        extendedEfficiencyBase: 0.35,
        extendedEfficiencyPerCom: 0.03,
        extendedEfficiencyCap: 0.75,
      },
    });
    ...
    const normalizeResourceMap = (value) => {
      ...
      resourceKeys.forEach((key) => {
        next[key] = Math.max(0, toSafeInteger(source[key], 0));
      });
      return next;
    };

    const loadEconomyV01Config = async () => {
      try {
        const response = await fetch('./config/economy.v0.1.json', { cache: 'no-store' });
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const parsed = await response.json();
        if (!parsed || typeof parsed !== 'object') throw new Error('Invalid config payload');
        return parsed;
      } catch (error) {
        console.warn('economy.v0.1 config fallback active', error);
        return buildFallbackEconomyConfig();
      }
    };

    const readScheduleResourceCost = (schedule, targetLevel, action) => {
    ...
    const readScheduleSeconds = (schedule, targetLevel, action) => {
      ...
      const rawIndex = action === 'upgrade' ? targetLevel - 2 : targetLevel - 1;
      const index = clamp(rawIndex, 0, schedule.length - 1);
      return Math.max(0, toSafeInteger(schedule[index], 0));
    };

    const readPerLevelNumber = (schedule, level) => {
      if (!Array.isArray(schedule) || !schedule.length) return 0;
      const index = clamp(level - 1, 0, schedule.length - 1);
      return Math.max(0, toSafeNumber(schedule[index], 0));
    };

    const weightedPickClass = (weights, rng) => {
      const entries = Object.entries(weights).filter(([, weight]) => Number.isFinite(weight) && weight > 0);
      if (!entries.length) return classOrder[0];
      const total = entries.reduce((sum, [, weight]) => sum + weight, 0);
      let cursor = rng() * total;
      for (const [classId, weight] of entries) {
        cursor -= weight;
        if (cursor <= 0) return classId;
      }
      return entries[entries.length - 1][0];
    };

    const assignClassByBio = (characterKey, username) => {
      const normalizedCharacter = normalizeCharacterKey(characterKey);
      const text = String(charactersByBio[normalizedCharacter] || normalizedCharacter).toLowerCase();
      const rng = createSeededRng(`${username}|${normalizedCharacter}|class-v01`);
      const weights = {};
      classOrder.forEach((classId) => {
        const classConfig = classDefinitions[classId];
        let score = 1 + (rng() * 0.35);
        classConfig.keywords.forEach((keyword) => {
          if (text.includes(keyword)) score += 1.45;
        });
        weights[classId] = score;
      });
      return weightedPickClass(weights, rng);
    };

    const createDefaultBuildingState = (buildingKey, now, startsBuilt) => ({
      id: `bld_${buildingKey}_${buildRandomHex(8)}`,
      key: buildingKey,
      level: startsBuilt ? 1 : 0,
      lastCollectedAt: now,
      status: 'active',
      statusUntil: 0,
    ...
    const createDefaultEconomyV01State = (selectedCharacter, config, now) => {
      const productionKeys = Object.keys(config.productionBuildings || {});
      const advancedKeys = Object.keys(config.advancedBuildings || {});
      const allKeys = [...productionKeys, ...advancedKeys];
      const startsBuilt = new Set(['farm', 'workshop', 'generator', 'data_center', 'hq', 'warehouse']);
      const buildings = {};
      allKeys.forEach((key) => {
        buildings[key] = createDefaultBuildingState(key, now, startsBuilt.has(key));
      });
      const startingTotalLevels = Object.values(buildings).reduce(
        (sum, building) => sum + Math.max(0, toSafeInteger(building.level, 0)),
        0,
      );

      return {
        version: 1,
        username: BUILDINGS_LAB_USERNAME,
        activeCharacter: normalizeVisibleCharacterKey(selectedCharacter, BUILDINGS_LAB_DEFAULT_CHARACTER) || BUILDINGS_LAB_DEFAULT_CHARACTER,
        inventory: {
          food: 260,
          materials: 220,
          energy: 190,
        ...
        progression: {
          startingTotalLevels,
          totalLevelMilestonesGranted: 0,
          hqRewardsGranted: [],
        },
        respec: {
          lastFreeRespecSeason: '',
        },
        log: [],
        dev: {
          timeOffsetMs: 0,
    ...
    const sanitizeBuildingState = (value, key, buildingConfig, now, defaultState) => {
      const source = value && typeof value === 'object' ? value : {};
      const maxLevel = Math.max(1, toSafeInteger(buildingConfig.maxLevel, 10));
      const level = clamp(toSafeInteger(source.level, defaultState.level), 0, maxLevel);
      const status = allowedBuildingStatuses.has(source.status) ? source.status : 'active';
      const statusUntil = Math.max(0, toSafeInteger(source.statusUntil, 0));
      return {
        id: typeof source.id === 'string' && source.id.trim() ? source.id.trim() : defaultState.id,
        key,
        level,
        lastCollectedAt: Math.max(0, toSafeInteger(source.lastCollectedAt, now)),
        status: level === 0 && status === 'upgrading' ? 'active' : status,
        statusUntil: status === 'active' ? 0 : statusUntil,
        specializationId: typeof source.specializationId === 'string' && source.specializationId.trim()
          ? source.specializationId.trim()
          : null,
        totalCollected: Math.max(0, toSafeNumber(source.totalCollected, 0)),
      };
    };

    const ensureProfileDailyState = (profile, now) => {
      const dayKey = getBerlinDayKey(now);
      const source = profile.daily && typeof profile.daily === 'object' ? profile.daily : {};
      if (source.dayKey !== dayKey) {
        profile.daily = {
          dayKey,
          harvestSurgeArmed: false,
          harvestSurgeUsed: false,
          balancedBudgetUsed: false,
        };
        return profile.daily;
      }
      profile.daily = {
        dayKey,
        harvestSurgeArmed: Boolean(source.harvestSurgeArmed),
        harvestSurgeUsed: Boolean(source.harvestSurgeUsed),
        balancedBudgetUsed: Boolean(source.balancedBudgetUsed),
      };
      return profile.daily;
    };

    const ensureCharacterProfile = (economy, characterKey, now) => {
      const normalizedCharacter = normalizeVisibleCharacterKey(characterKey, BUILDINGS_LAB_DEFAULT_CHARACTER) || BUILDINGS_LAB_DEFAULT_CHARACTER;
      if (!economy.characterProfiles || typeof economy.characterProfiles !== 'object') {
        economy.characterProfiles = {};
      }
      const existing = economy.characterProfiles[normalizedCharacter];
      if (existing && typeof existing === 'object') {
        const classId = classDefinitions[existing.classId] ? existing.classId : assignClassByBio(normalizedCharacter, economy.username);
        const baseStats = normalizeStatBlock(existing.baseStats, classDefinitions[classId].startStats);
        const allocatedStats = normalizeStatBlock(existing.allocatedStats, { AGR: 0, FAB: 0, PWR: 0, COM: 0, LOG: 0 });
        economy.characterProfiles[normalizedCharacter] = {
          characterKey: normalizedCharacter,
          classId,
          baseStats,
          allocatedStats,
          daily: existing.daily,
        };
        ensureProfileDailyState(economy.characterProfiles[normalizedCharacter], now);
        return economy.characterProfiles[normalizedCharacter];
      }
      const classId = assignClassByBio(normalizedCharacter, economy.username);
      economy.characterProfiles[normalizedCharacter] = {
        characterKey: normalizedCharacter,
        classId,
        baseStats: { ...classDefinitions[classId].startStats },
        allocatedStats: { AGR: 0, FAB: 0, PWR: 0, COM: 0, LOG: 0 },
        daily: {
          dayKey: getBerlinDayKey(now),
          harvestSurgeArmed: false,
          harvestSurgeUsed: false,
          balancedBudgetUsed: false,
        },
    ...
    const sanitizeEconomyV01State = (value, config, selectedCharacter, now) => {
      const base = createDefaultEconomyV01State(selectedCharacter, config, now);
      const source = value && typeof value === 'object' ? value : {};
      const productionKeys = Object.keys(config.productionBuildings || {});
      const advancedKeys = Object.keys(config.advancedBuildings || {});
      const allKeys = [...productionKeys, ...advancedKeys];
      ...
      allKeys.forEach((key) => {
        const buildingConfig = config.productionBuildings[key] || config.advancedBuildings[key] || {};
        const fallbackBuilding = base.buildings[key] || createDefaultBuildingState(key, now, false);
        mergedBuildings[key] = sanitizeBuildingState(
          source.buildings && source.buildings[key],
          key,
          buildingConfig,
          now,
          fallbackBuilding,
      ...
      const safeProgression = source.progression && typeof source.progression === 'object' ? source.progression : {};
      const safeRespec = source.respec && typeof source.respec === 'object' ? source.respec : {};
      const sanitized = {
        version: 1,
        username: String(source.username || BUILDINGS_LAB_USERNAME).toUpperCase(),
        activeCharacter: normalizeVisibleCharacterKey(
          source.activeCharacter || selectedCharacter || base.activeCharacter,
          base.activeCharacter,
        ) || base.activeCharacter,
        inventory: normalizeResourceMap(source.inventory || base.inventory),
        unspentStatPoints: Math.max(0, toSafeInteger(source.unspentStatPoints, 0)),
        buildings: mergedBuildings,
        characterProfiles: source.characterProfiles && typeof source.characterProfiles === 'object'
          ? deepClone(source.characterProfiles)
          : {},
        progression: {
          startingTotalLevels: Math.max(0, toSafeInteger(safeProgression.startingTotalLevels, base.progression.startingTotalLevels)),
          totalLevelMilestonesGranted: Math.max(0, toSafeInteger(safeProgression.totalLevelMilestonesGranted, 0)),
          hqRewardsGranted: Array.isArray(safeProgression.hqRewardsGranted)
            ? safeProgression.hqRewardsGranted
              .map((entry) => toSafeInteger(entry, 0))
              .filter((entry) => entry >= 2 && entry <= 5)
            : [],
        },
        respec: {
          lastFreeRespecSeason: typeof safeRespec.lastFreeRespecSeason === 'string'
            ? safeRespec.lastFreeRespecSeason
            : '',
        },
        log: Array.isArray(source.log) ? source.log.slice(0, 120) : [],
        dev: {
          timeOffsetMs: toSafeInteger(source.dev && source.dev.timeOffsetMs, 0),
          autoCollectEnabled: true,
        },
      };
      Object.keys(charactersByBio).filter(isCharacterVisible).forEach((characterKey) => {
        ensureCharacterProfile(sanitized, characterKey, now);
      });
      ensureCharacterProfile(sanitized, sanitized.activeCharacter, now);
      return sanitized;
    };

    const getSeasonKey = (now) => {
      const dayKey = getBerlinDayKey(now);
      const [yearText, monthText] = dayKey.split('-');
      const year = toSafeInteger(yearText, 2026);
      const month = clamp(toSafeInteger(monthText, 1), 1, 12);
      const quarter = Math.floor((month - 1) / 3) + 1;
      return `${year}-Q${quarter}`;
    };

    const getCurrentNow = (economy) => Date.now() + toSafeInteger(economy.dev && economy.dev.timeOffsetMs, 0);

    const getActiveProfile = (economy, now) => ensureCharacterProfile(
      economy,
      economy.activeCharacter || BUILDINGS_LAB_DEFAULT_CHARACTER,
      now,
    );

    const getEffectiveStats = (profile) => {
      const stats = {};
      statKeys.forEach((key) => {
        const base = toSafeInteger(profile.baseStats[key], 0);
        const allocated = toSafeInteger(profile.allocatedStats[key], 0);
        stats[key] = clamp(base + allocated, 0, 10);
      });
      return stats;
    };

    const getBuildingConfigByKey = (config, buildingKey) => (
      config.productionBuildings[buildingKey] || config.advancedBuildings[buildingKey] || null
    );
    const isProductionBuilding = (config, buildingKey) => Boolean(config.productionBuildings[buildingKey]);

    const computeOutputMultiplier = (primaryStat, logStat) => Math.min(1.35, 1 + (0.025 * primaryStat) + (0.010 * logStat));
    const computeCostMultiplier = (isProduction, primaryStat, logStat) => {
      if (isProduction) return Math.max(0.80, 1 - (0.015 * primaryStat) - (0.005 * logStat));
      return Math.max(0.85, 1 - (0.015 * logStat));
    };
    const computeTimeMultiplier = (isProduction, primaryStat, logStat) => {
      if (isProduction) return Math.max(0.80, 1 - (0.015 * logStat) - (0.005 * primaryStat));
      return Math.max(0.80, 1 - (0.020 * logStat));
    };

    const getBuildingPrimaryStat = (buildingConfig, stats) => {
      const statKey = String(buildingConfig && buildingConfig.primaryStat || '').toUpperCase();
      return statKeys.includes(statKey) ? toSafeInteger(stats[statKey], 0) : 0;
    };
    const getEffectiveProductionCycleMs = (buildingConfig) => {
      const baseCycleMinutes = Math.max(1, toSafeInteger(buildingConfig && buildingConfig.cycleMinutes, 1));
      const baseCycleMs = baseCycleMinutes * 60 * 1000;
      const capSeconds = toSafeInteger(BUILDINGS_LAB_PAYOUT_CYCLE_TEST_CAP_SECONDS, 0);
      if (capSeconds <= 0) return baseCycleMs;
      return Math.min(baseCycleMs, capSeconds * 1000);
    };

    const getOfflineCapMinutes = (buildingKey, buildingConfig, stats, classId) => {
      const primaryStat = getBuildingPrimaryStat(buildingConfig, stats);
      let minutes = 720 + (30 * stats.LOG) + (15 * primaryStat);
      if (classId === 'grid_engineer' && buildingKey === 'generator') {
        minutes += 60;
      }
      return Math.min(1440, minutes);
    };

    const appendLog = (economy, message, now) => {
      if (!message) return;
      if (!Array.isArray(economy.log)) economy.log = [];
      economy.log.unshift({
        at: now,
        timeLabel: getBerlinTimeLabel(now),
        dayKey: getBerlinDayKey(now),
        message: String(message),
      });
      economy.log = economy.log.slice(0, 140);
    };

    const applyProgressionRewards = (economy, now) => {
      const rewards = [];
      const hqBuilding = economy.buildings.hq;
      const hqLevel = hqBuilding ? Math.max(0, toSafeInteger(hqBuilding.level, 0)) : 0;
      const rewardedLevels = new Set(
        Array.isArray(economy.progression.hqRewardsGranted) ? economy.progression.hqRewardsGranted : [],
      );
      for (let level = 2; level <= Math.min(5, hqLevel); level += 1) {
        if (!rewardedLevels.has(level)) {
          rewardedLevels.add(level);
          economy.unspentStatPoints += 2;
          rewards.push(`HQ Lv${level} completed: +2 stat points`);
        }
      }
      economy.progression.hqRewardsGranted = Array.from(rewardedLevels).sort((a, b) => a - b);

      const totalLevels = Object.values(economy.buildings).reduce(
        (sum, building) => sum + Math.max(0, toSafeInteger(building.level, 0)),
        0,
      );
      const gainedLevels = Math.max(0, totalLevels - economy.progression.startingTotalLevels);
      const milestoneCount = Math.floor(gainedLevels / 10);
      if (milestoneCount > economy.progression.totalLevelMilestonesGranted) {
        const gain = milestoneCount - economy.progression.totalLevelMilestonesGranted;
        economy.progression.totalLevelMilestonesGranted = milestoneCount;
        economy.unspentStatPoints += gain;
        rewards.push(`Building level milestone reached: +${gain} stat point${gain > 1 ? 's' : ''}`);
      }
      rewards.forEach((reward) => appendLog(economy, reward, now));
    };

    const resolveFinishedJobs = (economy, config, now) => {
      let changed = false;
      Object.entries(economy.buildings).forEach(([buildingKey, building]) => {
        if (!building || building.status === 'active') return;
        const statusUntil = Math.max(0, toSafeInteger(building.statusUntil, 0));
        if (!statusUntil || now < statusUntil) return;

        const buildingConfig = getBuildingConfigByKey(config, buildingKey) || {};
        const maxLevel = Math.max(1, toSafeInteger(buildingConfig.maxLevel, 99));
        if (building.status === 'constructing' && building.level === 0) {
          building.level = 1;
        } else if (building.status === 'upgrading') {
          building.level = clamp(toSafeInteger(building.level, 0) + 1, 0, maxLevel);
        }
        building.status = 'active';
        building.statusUntil = 0;
        building.lastCollectedAt = Math.max(building.lastCollectedAt, statusUntil);
        changed = true;
        appendLog(
          economy,
          `${buildingConfig.name || buildingKey} is now level ${building.level} and active.`,
          now,
        );
      });
      if (changed) applyProgressionRewards(economy, now);
      return changed;
    };

    const getAutomationExtensionContext = (economy, config, stats, cycleMs) => {
      const automationLevel = Math.max(0, toSafeInteger(economy.buildings.automation && economy.buildings.automation.level, 0));
      if (automationLevel <= 0) {
        return { maxCyclesWithAutomation: 0, extendedEfficiency: 0 };
      }
      const automationConfig = config.automation || {};
      const extraMinutes = Math.max(
          0,
          toSafeNumber(automationConfig.extendedCapMinutesBase, 0)
            + (Math.max(0, automationLevel - 1) * toSafeNumber(automationConfig.extendedCapMinutesPerLevel, 0)),
      );
      const extraMs = extraMinutes * 60 * 1000;
      const extraCycles = Math.max(0, Math.floor(extraMs / Math.max(1000, cycleMs)));
      const extendedEfficiency = clamp(
        toSafeNumber(automationConfig.extendedEfficiencyBase, 0.35)
          + (toSafeNumber(automationConfig.extendedEfficiencyPerCom, 0.03) * stats.COM),
        0,
        toSafeNumber(automationConfig.extendedEfficiencyCap, 0.75),
      );
      return {
        maxCyclesWithAutomation: extraCycles,
        extendedEfficiency,
      };
    };

    const computeLazyCycleWindow = (elapsedMs, cycleMs, offlineCapMinutes, extraAutomationCycles = 0) => {
      const safeCycleMs = Math.max(1000, toSafeInteger(cycleMs, 1000));
      const safeElapsedMs = Math.max(0, toSafeInteger(elapsedMs, 0));
      const totalCycles = Math.floor(safeElapsedMs / safeCycleMs);
      const fullCyclesCap = Math.max(0, Math.floor((Math.max(0, toSafeNumber(offlineCapMinutes, 0)) * 60 * 1000) / safeCycleMs));
      const maxCyclesWithAutomation = fullCyclesCap + Math.max(0, toSafeInteger(extraAutomationCycles, 0));
      const cyclesAtFull = Math.min(totalCycles, fullCyclesCap);
      const cyclesExtended = Math.max(0, Math.min(totalCycles, maxCyclesWithAutomation) - cyclesAtFull);
      const consumedCycles = cyclesAtFull + cyclesExtended;
      const cycleRemainderMs = safeElapsedMs % safeCycleMs;
      const nextCycleInMs = cycleRemainderMs === 0 ? safeCycleMs : safeCycleMs - cycleRemainderMs;
      return {
        totalCycles,
        fullCyclesCap,
        maxCyclesWithAutomation,
        cyclesAtFull,
        cyclesExtended,
        consumedCycles,
        nextCycleInMs,
    ...
    const previewCollection = (economy, config, buildingKey, now) => {
      const building = economy.buildings[buildingKey];
      const buildingConfig = config.productionBuildings[buildingKey];
      if (!building || !buildingConfig) return null;
      if (building.level <= 0) return null;
      if (building.status !== 'active') return null;

      const profile = getActiveProfile(economy, now);
      const stats = getEffectiveStats(profile);
      const classId = profile.classId;
      const cycleMs = getEffectiveProductionCycleMs(buildingConfig);
      const cycleMinutes = cycleMs / (60 * 1000);
      const primaryStat = getBuildingPrimaryStat(buildingConfig, stats);
      const outputMultiplier = computeOutputMultiplier(primaryStat, stats.LOG);
      const baseOutput = readPerLevelNumber(buildingConfig.baseOutputPerCycle, building.level);
      const perCycleOutput = Math.floor(baseOutput * outputMultiplier * 1);
      const elapsedMs = Math.max(0, now - building.lastCollectedAt);
      const offlineCapMinutes = getOfflineCapMinutes(buildingKey, buildingConfig, stats, classId);
      const automation = getAutomationExtensionContext(economy, config, stats, cycleMs);
      const cycleWindow = computeLazyCycleWindow(
        elapsedMs,
        cycleMs,
        offlineCapMinutes,
        automation.maxCyclesWithAutomation,
      );
      if (cycleWindow.totalCycles <= 0) {
        return {
          cycleMinutes,
          totalCycles: 0,
          cyclesAtFull: 0,
          ...
          perCycleOutput,
          projectedOutput: 0,
          nextCycleInMs: cycleWindow.nextCycleInMs,
          offlineCapMinutes,
        };
      }

      const fullOutput = cycleWindow.cyclesAtFull * perCycleOutput;
      const extendedOutput = Math.floor(
        cycleWindow.cyclesExtended * perCycleOutput * automation.extendedEfficiency,
      );
      let projectedOutput = fullOutput + extendedOutput;

      const daily = ensureProfileDailyState(profile, now);
      if (classId === 'homesteader' && buildingKey === 'farm' && daily.harvestSurgeArmed && !daily.harvestSurgeUsed) {
        projectedOutput += Math.floor(projectedOutput * 0.20);
      }
      if (classId === 'data_architect' && buildingKey === 'data_center') {
        const tenMinutes = 10 * 60 * 1000;
        if (elapsedMs >= cycleMs && (elapsedMs % cycleMs) <= tenMinutes) {
          projectedOutput += Math.floor((fullOutput + extendedOutput) * 0.10);
        }
      }

      return {
        cycleMinutes,
        totalCycles: cycleWindow.totalCycles,
        cyclesAtFull: cycleWindow.cyclesAtFull,
        cyclesExtended: cycleWindow.cyclesExtended,
        perCycleOutput,
        projectedOutput,
        nextCycleInMs: cycleWindow.nextCycleInMs,
        offlineCapMinutes,
    ...
    const collectFromBuilding = (economy, config, buildingKey, now) => {
      const building = economy.buildings[buildingKey];
      const buildingConfig = config.productionBuildings[buildingKey];
      if (!building || !buildingConfig) return { ok: false, message: 'Building is not a production type.' };
      if (building.level <= 0) return { ok: false, message: `${buildingConfig.name} is not built yet.` };
      if (building.status !== 'active') {
        return { ok: true, gained: 0, wasted: 0, message: `${buildingConfig.name}: ${building.status}, no collection yet.` };
      }
      ...
      if (cycleWindow.totalCycles <= 0) {
        return { ok: true, gained: 0, wasted: 0, message: `${buildingConfig.name}: no full cycles yet.` };
      }
      ...
      const baseOutput = readPerLevelNumber(buildingConfig.baseOutputPerCycle, building.level);
      const perCycleOutput = Math.floor(baseOutput * outputMultiplier * 1);
      const outputFull = cycleWindow.cyclesAtFull * perCycleOutput;
      const outputExtended = Math.floor(
        cycleWindow.cyclesExtended * perCycleOutput * automation.extendedEfficiency,
      );
      let outputTotal = outputFull + outputExtended;

      const daily = ensureProfileDailyState(profile, now);
      if (classId === 'homesteader' && buildingKey === 'farm' && daily.harvestSurgeArmed && !daily.harvestSurgeUsed) {
        outputTotal += Math.floor(outputTotal * 0.20);
        daily.harvestSurgeUsed = true;
        daily.harvestSurgeArmed = false;
      }
      if (classId === 'data_architect' && buildingKey === 'data_center') {
        const tenMinutes = 10 * 60 * 1000;
        if (elapsedMs >= cycleMs && (elapsedMs % cycleMs) <= tenMinutes) {
          outputTotal += Math.floor((outputFull + outputExtended) * 0.10);
        }
      }

      const resourceKey = String(buildingConfig.resource || '').toLowerCase();
      if (!resourceKeys.includes(resourceKey)) {
        return { ok: false, message: `${buildingConfig.name}: invalid output resource.` };
      }
      const currentAmount = Math.max(0, toSafeInteger(economy.inventory[resourceKey], 0));
      const gained = Math.max(0, outputTotal);
      const wasted = 0;
      economy.inventory[resourceKey] = currentAmount + gained;

      const consumedCycles = cycleWindow.consumedCycles;
      if (consumedCycles > 0) {
        building.lastCollectedAt += consumedCycles * cycleMs;
      }
      building.totalCollected = Math.max(0, toSafeNumber(building.totalCollected, 0) + gained);
      return {
        ok: true,
        gained,
        wasted,
        resourceKey,
        message: `${buildingConfig.name}: +${formatNumber(gained)} ${resourceKey.toUpperCase()}`,
      };
    };

    const autoCollectDueProduction = (economy, config, now) => {
      let changed = false;
      const productionBuildings = config.productionBuildings || {};
      Object.keys(productionBuildings).forEach((buildingKey) => {
        const building = economy.buildings[buildingKey];
        if (!building || building.level <= 0 || building.status !== 'active') return;
        const preview = previewCollection(economy, config, buildingKey, now);
        if (!preview || preview.totalCycles <= 0) return;
        const result = collectFromBuilding(economy, config, buildingKey, now);
        if (result.ok && (result.gained > 0 || result.wasted > 0)) changed = true;
      });
      return changed;
    };

    const getBuilderCrewLimit = (economy) => {
      const contractorLevel = Math.max(0, toSafeInteger(economy.buildings.contractor_office && economy.buildings.contractor_office.level, 0));
      return Math.max(1 + contractorLevel, BUILDINGS_LAB_BUILDER_CREW_OVERRIDE);
    };

    const countActiveJobs = (economy, now) => Object.values(economy.buildings).filter(
      (building) => building.status !== 'active' && now < toSafeInteger(building.statusUntil, 0),
    ).length;

    const startBuildOrUpgrade = (economy, config, buildingKey, now) => {
      const building = economy.buildings[buildingKey];
      const buildingConfig = getBuildingConfigByKey(config, buildingKey);
      if (!building || !buildingConfig) return { ok: false, message: 'Unknown building.' };
      if (building.status !== 'active') return { ok: false, message: `${buildingConfig.name} already has an active job.` };
      const estimate = computeNextJobEstimate(economy, config, buildingKey, now);
      if (!estimate) return { ok: false, message: `${buildingConfig.name} reached max level.` };

      const activeJobs = countActiveJobs(economy, now);
      const crewLimit = getBuilderCrewLimit(economy);
      if (activeJobs >= crewLimit) {
        return { ok: false, message: `Builder crews full (${activeJobs}/${crewLimit}). Upgrade Contractor Desk.` };
      }

      for (const resourceKey of resourceKeys) {
        const owned = Math.max(0, toSafeInteger(economy.inventory[resourceKey], 0));
        if (owned < estimate.finalCost[resourceKey]) {
          return {
            ok: false,
            message: `${buildingConfig.name}: not enough ${resourceKey}. Need ${formatNumber(estimate.finalCost[resourceKey])}.`,
          };
        }
      }
      ...
      if (estimate.willConsumeBalancedBudget) {
        const profile = getActiveProfile(economy, now);
        const daily = ensureProfileDailyState(profile, now);
        daily.balancedBudgetUsed = true;
      }

      building.status = estimate.action === 'build' ? 'constructing' : 'upgrading';
      building.statusUntil = now + (estimate.finalSeconds * 1000);
    ...
    const allocateStatPoint = (economy, statKey, now) => {
      if (!statKeys.includes(statKey)) return false;
      if (economy.unspentStatPoints <= 0) return false;
      const profile = getActiveProfile(economy, now);
      const effectiveStats = getEffectiveStats(profile);
      if (effectiveStats[statKey] >= 10) return false;
      profile.allocatedStats[statKey] = Math.max(0, toSafeInteger(profile.allocatedStats[statKey], 0) + 1);
      economy.unspentStatPoints -= 1;
      appendLog(economy, `Allocated +1 ${statKey}.`, now);
      return true;
    };

    const runFreeRespec = (economy, now) => {
      const profile = getActiveProfile(economy, now);
      const seasonKey = getSeasonKey(now);
      if (economy.respec.lastFreeRespecSeason === seasonKey) return false;
      const refund = statKeys.reduce((sum, key) => sum + Math.max(0, toSafeInteger(profile.allocatedStats[key], 0)), 0);
      statKeys.forEach((key) => {
        profile.allocatedStats[key] = 0;
      });
      economy.unspentStatPoints += refund;
      economy.respec.lastFreeRespecSeason = seasonKey;
      appendLog(economy, `Free seasonal respec used for ${seasonKey}. Refunded ${refund} points.`, now);
      return true;
    };

    const resetCollectedInventory = (economy, config, now) => {
      resourceKeys.forEach((resourceKey) => {
        economy.inventory[resourceKey] = 0;
      });
      Object.keys(config.productionBuildings || {}).forEach((buildingKey) => {
        const building = economy.buildings[buildingKey];
        if (!building) return;
        building.lastCollectedAt = now;
        building.totalCollected = 0;
      });
      appendLog(economy, 'Collected inventory reset.', now);
    };

    const computeNextJobEstimate = (economy, config, buildingKey, now) => {
      const building = economy.buildings[buildingKey];
      const buildingConfig = getBuildingConfigByKey(config, buildingKey);
      if (!building || !buildingConfig) return null;

      const maxLevel = Math.max(1, toSafeInteger(buildingConfig.maxLevel, 10));
      const action = building.level <= 0 ? 'build' : 'upgrade';
      const targetLevel = action === 'build' ? 1 : building.level + 1;
      if (targetLevel > maxLevel) return null;

      const profile = getActiveProfile(economy, now);
      const stats = getEffectiveStats(profile);
      const daily = ensureProfileDailyState(profile, now);
      const production = isProductionBuilding(config, buildingKey);
      const primaryStat = getBuildingPrimaryStat(buildingConfig, stats);
      const costFloor = production ? 0.80 : 0.85;
      const costRaw = production
        ? 1 - (0.015 * primaryStat) - (0.005 * stats.LOG)
        : 1 - (0.015 * stats.LOG);
      const costMultiplier = Math.max(costFloor, costRaw);
      const timeFloor = 0.80;
      const timeRaw = production
        ? 1 - (0.015 * stats.LOG) - (0.005 * primaryStat)
        : 1 - (0.020 * stats.LOG);
      const timeMultiplier = Math.max(timeFloor, timeRaw);

      const baseCost = readScheduleResourceCost(
        action === 'build' ? buildingConfig.buildCost : buildingConfig.upgradeCost,
        targetLevel,
        action,
      );
      const baseCash = Math.max(0, toSafeInteger(baseCost.cash, 0));
      const finalCost = {};
      resourceKeys.forEach((key) => {
        finalCost[key] = Math.ceil(baseCost[key] * costMultiplier);
      });
      let finalCash = Math.ceil(baseCash * costMultiplier);

      const adjustments = [];
      if (profile.classId === 'factory_foreman' && buildingKey === 'workshop' && action === 'upgrade') {
        finalCost.materials = Math.ceil(finalCost.materials * 0.95);
        adjustments.push('Factory Foreman trait: -5% Materials (Workshop upgrade)');
      }

      const willConsumeBalancedBudget = profile.classId === 'administrator' && !daily.balancedBudgetUsed;
      if (willConsumeBalancedBudget) {
        resourceKeys.forEach((key) => {
          finalCost[key] = Math.ceil(finalCost[key] * 0.95);
        });
        finalCash = Math.ceil(finalCash * 0.95);
        adjustments.push('Administrator trait: -5% all resources (first build/upgrade today)');
      }

      const baseSeconds = readScheduleSeconds(
        action === 'build' ? buildingConfig.buildSeconds : buildingConfig.upgradeSeconds,
        targetLevel,
        action,
      );
      const scaledSeconds = Math.max(0, Math.ceil(baseSeconds * timeMultiplier));
      const finalSeconds = Math.min(BUILDINGS_LAB_BUILD_TIME_TEST_CAP_SECONDS, scaledSeconds);
      if (scaledSeconds > BUILDINGS_LAB_BUILD_TIME_TEST_CAP_SECONDS) {
        adjustments.push(`Test cap active: build time limited to ${BUILDINGS_LAB_BUILD_TIME_TEST_CAP_SECONDS}s`);
      }
      return {
        action,
        targetLevel,
        ...
        baseSeconds,
        finalSeconds,
        willConsumeBalancedBudget,
        adjustments,
      };
    };

    const renderLogPanel = (economy) => {
      if (!buildingsLabLog) return;
      const entries = Array.isArray(economy.log) ? economy.log : [];
      if (!entries.length) {
        buildingsLabLog.innerHTML = '<div class="muted">No messages yet.</div>';
        return;
      }
      buildingsLabLog.innerHTML = entries.map((entry) => {
        const safeMessage = String(entry.message || '').replace(/[<>&]/g, (char) => (
          char === '<' ? '&lt;' : char === '>' ? '&gt;' : '&amp;'
        ));
        const safeTime = String(entry.timeLabel || '--:--:--').replace(/[<>&]/g, '');
    ...
    const toHtmlAttributeText = (value) => escapeHtml(String(value || '')).replace(/\n/g, '&#10;');

    const buildJobTooltipText = (economy, config, buildingKey, nextJob) => {
      const buildingConfig = getBuildingConfigByKey(config, buildingKey);
      const buildingName = buildingConfig && buildingConfig.name ? buildingConfig.name : buildingKey;
      if (!nextJob) return `${buildingName}: Max level reached`;

      const playerSave = getPlayerSave();
      const cashOwned = Math.max(0, toSafeNumber(playerSave && playerSave.wallet && playerSave.wallet.cash, 0));
      const lines = [
        `${buildingName}: ${nextJob.action === 'build' ? 'Build' : 'Upgrade'} to Lv ${nextJob.targetLevel}`,
        'You need:',
      ];

      resourceKeys.forEach((resourceKey) => {
        const label = (config.resources[resourceKey] && config.resources[resourceKey].label) || resourceKey.toUpperCase();
        const need = Math.max(0, toSafeInteger(nextJob.finalCost[resourceKey], 0));
        const have = Math.max(0, toSafeInteger(economy.inventory[resourceKey], 0));
        lines.push(`- ${label}: need ${formatNumber(need)} | have ${formatNumber(have)}`);
      });

      lines.push(`- Cash: need ${formatCashLabel(nextJob.finalCash)} | have ${formatCashLabel(cashOwned)}`);
      lines.push(`Time: ${formatDurationMs(nextJob.finalSeconds * 1000)}`);
      return lines.join('\n');
    };

    const renderLab = (state) => {
      const now = getCurrentNow(state.economy);
      if (!state.economy.dev || typeof state.economy.dev !== 'object') {
        state.economy.dev = {};
      }
      state.economy.dev.autoCollectEnabled = true;
      const autoCollectEnabled = true;
      let renderMutatedEconomy = false;
      if (resolveFinishedJobs(state.economy, state.config, now)) {
        renderMutatedEconomy = true;
      }
      if (autoCollectDueProduction(state.economy, state.config, now)) {
        renderMutatedEconomy = true;
      }
      if (renderMutatedEconomy) {
        state.persist();
      }
      const profile = getActiveProfile(state.economy, now);
      const classConfig = classDefinitions[profile.classId];
      const stats = getEffectiveStats(profile);
      const daily = ensureProfileDailyState(profile, now);
      const productionKeys = Object.keys(state.config.productionBuildings || {});
      const advancedKeys = Object.keys(state.config.advancedBuildings || {});
      const crewLimit = getBuilderCrewLimit(state.economy);
      const activeJobs = countActiveJobs(state.economy, now);

      const previousInventory = state.inventoryRenderSnapshot && typeof state.inventoryRenderSnapshot === 'object'
        ? state.inventoryRenderSnapshot
        : null;
      const nextInventorySnapshot = {};
      const inventoryRows = resourceKeys.map((resourceKey) => {
        const current = Math.max(0, toSafeInteger(state.economy.inventory[resourceKey], 0));
        nextInventorySnapshot[resourceKey] = current;
        const previous = previousInventory
          ? Math.max(0, toSafeInteger(previousInventory[resourceKey], current))
          : current;
        const gain = Math.max(0, current - previous);
        const label = (state.config.resources[resourceKey] && state.config.resources[resourceKey].label) || resourceKey;
        return `
          <div class="bld-resource">
            <div class="bld-resource-main">
              <div class="bld-resource-label">${escapeHtml(label)}</div>
              <div class="bld-resource-value${gain > 0 ? ' bld-resource-value-gain' : ''}">${formatNumber(current)}</div>
              <div class="bld-resource-delta${gain > 0 ? '' : ' bld-resource-delta-empty'}">${gain > 0 ? `+${formatNumber(gain)}` : ''}</div>
            </div>
      ...
      const statRows = statKeys.map((statKey) => {
        const value = stats[statKey];
        const widthPercent = Math.round((value / 10) * 100);
        const canAllocate = state.economy.unspentStatPoints > 0 && value < 10;
        return `
          <div class="bld-stat-row">
            ...
            <button type="button" class="bld-btn alt" data-action="allocate-stat" data-stat="${statKey}" ${canAllocate ? '' : 'disabled'}>+1</button>
          </div>
        `;
      }).join('');

      const facilityCards = productionKeys.map((buildingKey) => {
        const building = state.economy.buildings[buildingKey];
        const config = state.config.productionBuildings[buildingKey];
        const preview = previewCollection(state.economy, state.config, buildingKey, now);
        const statusLabel = building.status === 'active'
          ? 'READY'
          : (building.status === 'constructing' ? 'BUILDING' : 'UPGRADING');
        const resourceKey = String(config.resource || '').toLowerCase();
        const nextJob = computeNextJobEstimate(state.economy, state.config, buildingKey, now);
        const actionLabel = nextJob ? (nextJob.action === 'build' ? 'Build' : 'Upgrade') : 'Max';
        const canJob = building.status === 'active' && Boolean(nextJob);
        const jobTooltip = toHtmlAttributeText(buildJobTooltipText(state.economy, state.config, buildingKey, nextJob));
        const hasValidPayoutResource = resourceKeys.includes(resourceKey);
        const payoutLabel = hasValidPayoutResource
          ? ((state.config.resources[resourceKey] && state.config.resources[resourceKey].label) || resourceKey.toUpperCase())
          : 'Output';
        let payoutPerCycle = 0;
        ...
        if (building.level <= 0) {
          payoutEtaText = '-';
          readyNowText = 'Build this first';
        } else if (building.status !== 'active') {
          payoutEtaText = statusLabel;
          readyNowText = `Done in ${formatDurationMs(Math.max(0, building.statusUntil - now))}`;
        } else if (preview) {
          payoutPerCycle = Math.max(0, toSafeInteger(preview.perCycleOutput, 0));
          payoutEtaText = formatDurationMs(Math.max(0, toSafeInteger(preview.nextCycleInMs, 0)));
          if (preview.projectedOutput > 0) {
            readyNowText = `Auto collect pending: ${formatNumber(preview.projectedOutput)} ${resourceKey.toUpperCase()}`;
          } else {
            readyNowText = 'Next cycle not finished yet';
          }
        }
        const payoutBreakdownRows = hasValidPayoutResource
          ? `
            <div class="bld-facility-payout">
              <span class="bld-facility-payout-label">${escapeHtml(payoutLabel)}</span>
              <span class="bld-facility-payout-value">${formatNumber(payoutPerCycle)}</span>
            </div>
          `
          : `<div class="bld-mini-note">No payout resource configured.</div>`;

        return `
          <article class="bld-facility-card">
            <div class="bld-facility-main">
              <div class="bld-facility-name">${escapeHtml(config.name)} <span class="bld-sub">Lv ${building.level}</span></div>
              <div class="bld-facility-status"><span class="bld-status ${building.status}">${statusLabel}</span></div>
              <div class="bld-facility-line"><strong>Next payout in:</strong> ${escapeHtml(payoutEtaText)}</div>
              <div class="bld-facility-line"><strong>Each cycle gives:</strong></div>
              <div class="bld-facility-payouts">${payoutBreakdownRows}</div>
              <div class="bld-mini-note">${escapeHtml(readyNowText)}</div>
              <div class="bld-action-row">
                <button
                  type="button"
                  class="bld-btn"
                  data-action="job-one"
                  data-building="${buildingKey}"
                  data-job-tooltip="${jobTooltip}"
                  aria-label="${actionLabel} ${escapeHtml(config.name)}"
                  ${canJob ? '' : 'disabled'}
                >${actionLabel}</button>
              </div>
            </div>
            <span class="bld-facility-icon-slot bld-square-icon-slot" data-facility="${escapeHtml(buildingKey)}" aria-hidden="true"></span>
          </article>
        `;
      }).join('');

      const advancedCards = advancedKeys.map((buildingKey) => {
        const building = state.economy.buildings[buildingKey];
        const config = state.config.advancedBuildings[buildingKey];
        ...
        const canJob = building.status === 'active' && Boolean(nextJob);
        const jobTooltip = toHtmlAttributeText(buildJobTooltipText(state.economy, state.config, buildingKey, nextJob));
        const features = Array.isArray(config.sampleFeatures) && config.sampleFeatures.length
          ? config.sampleFeatures.slice(0, 2).join(' • ')
          : 'No feature details yet.';
        const staff = Array.isArray(config.sampleStaff) && config.sampleStaff.length
          ? config.sampleStaff.slice(0, 2).join(' • ')
          : 'No staffing details yet.';
        const anchorId = buildingKey === 'marketplace' ? ' id="marketplace-section"' : '';

        let readinessText = '';
        if (building.level <= 0) {
          readinessText = 'Build this facility first';
        } else if (building.status !== 'active') {
          readinessText = `Done in ${formatDurationMs(Math.max(0, building.statusUntil - now))}`;
        } else {
          readinessText = nextJob ? 'Ready for next upgrade' : 'Max level reached';
        }

        return `
          <article class="bld-facility-card"${anchorId}>
            <div class="bld-facility-main">
              <div class="bld-facility-name">${escapeHtml(config.name)} <span class="bld-sub">Lv ${building.level}</span></div>
              <div class="bld-facility-status"><span class="bld-status ${building.status}">${statusLabel}</span></div>
              <div class="bld-facility-line"><strong>Feature set:</strong> ${escapeHtml(features)}</div>
              <div class="bld-facility-line"><strong>Crew:</strong> ${escapeHtml(staff)}</div>
              <div class="bld-mini-note">${escapeHtml(readinessText)}</div>
              <div class="bld-action-row">
                <button
                  type="button"
                  class="bld-btn"
      ...
      const characterOptions = Object.keys(charactersByBio).filter(isCharacterVisible).map((characterKey) => (
        `<option value="${characterKey}" ${characterKey === state.economy.activeCharacter ? 'selected' : ''}>${characterKey.toUpperCase()}</option>`
      )).join('');

      const respecUsed = state.economy.respec.lastFreeRespecSeason === getSeasonKey(now);
      const statsPanelHtml = `
        <div class="bld-stats-shell">
          <div class="bld-stats-shell-main">
            <div class="bld-stats-grid">${statRows}</div>
            <div class="bld-action-row bld-side-actions">
              <button type="button" class="bld-btn alt" data-action="respec" ${respecUsed ? 'disabled' : ''}>Reset Stats (Free)</button>
            </div>
          </div>
        </div>
        <div class="bld-trait-shell">
          <div class="bld-trait">
            <strong>${escapeHtml(classConfig.traitName)}:</strong> ${escapeHtml(classConfig.traitDescription)}
            ${profile.classId === 'homesteader'
              ? `<div class="bld-action-row bld-side-actions">
                  <button type="button" class="bld-btn alt" data-action="arm-harvest" ${daily.harvestSurgeUsed ? 'disabled' : ''}>
                    ${daily.harvestSurgeArmed ? 'Turn Off Harvest Boost' : 'Turn On Harvest Boost'}
                  </button>
                  <span class="bld-mini-note">${daily.harvestSurgeUsed ? 'Used today' : 'Ready today'}</span>
                </div>`
              : ''
            }
            ${profile.classId === 'administrator'
              ? `<div class="bld-mini-note bld-side-note">Daily discount: ${daily.balancedBudgetUsed ? 'USED' : 'READY'}</div>`
              : ''
            }
          </div>
      ...
      `;
      if (showSidePanel && buildingsSideStats) {
        buildingsSideStats.innerHTML = statsPanelHtml;
      } else if (buildingsSideStats) {
      ...
      const toolbarHtml = showToolbar
        ? `
          <div class="bld-toolbar">
            <div class="bld-toolbar-item">Player <strong>${escapeHtml(state.economy.username)}</strong></div>
            <div class="bld-toolbar-item">Today <strong>${getBerlinDayKey(now)}</strong></div>
            <div class="bld-toolbar-item">Builders <strong>${activeJobs}/${crewLimit}</strong></div>
            <div class="bld-toolbar-item">Class <strong>${escapeHtml(classConfig.name)}</strong></div>
            <div class="bld-toolbar-item">Auto <strong>${autoCollectEnabled ? 'ON' : 'OFF'}</strong></div>
            <label class="bld-toolbar-item">Character
              <select class="bld-character-select" data-action="set-character">${characterOptions}</select>
            </label>
            <div class="bld-toolbar-item">Stat Points <strong>${state.economy.unspentStatPoints}</strong></div>
          </div>
        `
        : '';

      const resourcesSectionHtml = showResourcesSection
        ? `
          <div class="bld-grid">
            <section class="bld-card">
              <h3 class="bld-card-title">Resources</h3>
              <div class="bld-inventory-grid">${inventoryRows}</div>
              <div class="bld-action-row" style="margin-top:10px;">
                <button type="button" class="bld-btn alt" data-action="reset-inventory">Reset Inventory</button>
                <span class="bld-mini-note">Auto Collect is always ON in this simulator.</span>
              </div>
            </section>
          </div>
        `
      ...
      const productionSectionHtml = showProductionSection
        ? `
          <section class="bld-section">
            <div class="bld-section-head">
              <div class="hdr">Your 4 Buildings</div>
              <div class="bld-mini-note">Tap Build/Upgrade. Collection is automatic.</div>
            </div>
            <div class="bld-facility-grid">
              ${facilityCards}
            </div>
          </section>
        `
      ...
      const advancedSectionHtml = showAdvancedSection
        ? `
          <section class="bld-section" id="advanced-hq-section">
            <div class="bld-section-head">
              <div class="hdr">Advanced HQs</div>
              <div class="bld-mini-note">Strategic facilities for expansion and upgrades.</div>
            </div>
            <div class="bld-facility-grid">
              ${advancedCards}
            </div>
      ...
      buildingsLabRoot.innerHTML = `
        <div class="bld-lab-shell">
          ${toolbarHtml}
      ...
      if (showSidePanel) {
        renderLogPanel(state.economy);
      } else if (buildingsLabLog) {
        buildingsLabLog.innerHTML = '';
      }
      state.inventoryRenderSnapshot = nextInventorySnapshot;
    };
    ...
    const initBuildingsLab = async () => {
      ...
      sanitizedEconomy.username = BUILDINGS_LAB_USERNAME;
      if (!sanitizedEconomy.activeCharacter) sanitizedEconomy.activeCharacter = BUILDINGS_LAB_DEFAULT_CHARACTER;

      if (normalizeCharacterKey(currentSave.selectedCharacter) !== normalizeCharacterKey(sanitizedEconomy.activeCharacter)) {
        setSelectedCharacterInPlayerSave(sanitizedEconomy.activeCharacter);
      }

      const state = {
        config,
        economy: sanitizedEconomy,
        persist: () => {},
      };
      state.persist = () => {
        const nextSave = {
          ...getPlayerSave(),
          selectedCharacter: state.economy.activeCharacter,
      ...
      if (saveProgressButton) {
        const showSavedState = () => {
          ...
          saveProgressButton.classList.add('is-saved');
          window.setTimeout(() => {
            saveProgressButton.textContent = 'SAVE PROGRESS';
            saveProgressButton.classList.remove('is-saved');
          }, 1400);
        };
        saveProgressButton.addEventListener('click', () => {
          const now = getCurrentNow(state.economy);
          state.persist();
          appendLog(state.economy, 'Progress saved manually.', now);
          renderLab(state);
          showSavedState();
        });
      }
      ...
      tooltipEl.className = 'bld-hover-tooltip';
      ...
      const hideJobTooltip = () => {
        tooltipEl.classList.remove('is-visible');
        tooltipEl.innerHTML = '';
        tooltipState.text = '';
      };

      const positionJobTooltip = () => {
        const gap = 12;
        const rect = tooltipEl.getBoundingClientRect();
        let left = tooltipState.x + gap;
        let top = tooltipState.y + gap;
        if (left + rect.width > window.innerWidth - 8) {
          left = tooltipState.x - rect.width - gap;
        }
      ...
      const syncJobTooltipFromPointer = () => {
        if (!tooltipState.pointerActive) {
          hideJobTooltip();
          return;
        }
        const nodeAtPointer = document.elementFromPoint(tooltipState.x, tooltipState.y);
        const tooltipTarget = nodeAtPointer && nodeAtPointer.closest
          ? nodeAtPointer.closest('[data-job-tooltip]')
          : null;
        if (!tooltipTarget || !buildingsLabRoot.contains(tooltipTarget)) {
          hideJobTooltip();
          return;
        }
        const rawText = String(tooltipTarget.getAttribute('data-job-tooltip') || '').replace(/&#10;/g, '\n').trim();
        if (!rawText) {
          hideJobTooltip();
          return;
        }

        if (tooltipState.text !== rawText) {
          tooltipState.text = rawText;
          tooltipEl.innerHTML = escapeHtml(rawText).replace(/\n/g, '<br>');
        }
      ...
      const handleTooltipPointerMove = (event) => {
        tooltipState.pointerActive = true;
        tooltipState.x = event.clientX;
        tooltipState.y = event.clientY;
        syncJobTooltipFromPointer();
      };

      buildingsLabRoot.addEventListener('pointermove', handleTooltipPointerMove);
      buildingsLabRoot.addEventListener('pointerleave', () => {
        tooltipState.pointerActive = false;
        hideJobTooltip();
      });
      buildingsLabRoot.addEventListener('scroll', () => {
        if (tooltipState.pointerActive) syncJobTooltipFromPointer();
      }, { passive: true });
      ...
      const facilitiesTickerId = window.setInterval(() => {
        renderLab(state);
        syncJobTooltipFromPointer();
      }, 1000);
      const buildingsAutosaveId = window.setInterval(() => {
        state.persist();
      }, 5000);
      const persistBuildingsOnLifecycleEvent = () => {
        state.persist();
      };
      const handleBuildingsVisibilityChange = () => {
        if (document.visibilityState === 'hidden') {
          state.persist();
        }
      };
      window.addEventListener('pagehide', persistBuildingsOnLifecycleEvent, { passive: true });
      window.addEventListener('beforeunload', persistBuildingsOnLifecycleEvent, { passive: true });
      document.addEventListener('visibilitychange', handleBuildingsVisibilityChange, { passive: true });
      window.addEventListener('beforeunload', () => {
        window.clearInterval(facilitiesTickerId);
        window.clearInterval(buildingsAutosaveId);
        window.removeEventListener('pagehide', persistBuildingsOnLifecycleEvent);
        window.removeEventListener('beforeunload', persistBuildingsOnLifecycleEvent);
        document.removeEventListener('visibilitychange', handleBuildingsVisibilityChange);
        tooltipEl.remove();
      }, { once: true });

      const handleLabActionClick = (event) => {
        const target = event.target.closest('[data-action]');
        if (!target) return;
        const action = target.getAttribute('data-action');
        const now = getCurrentNow(state.economy);
        let shouldPersist = false;
        resolveFinishedJobs(state.economy, state.config, now);

        if (action === 'job-one') {
          const buildingKey = target.getAttribute('data-building') || '';
          const result = startBuildOrUpgrade(state.economy, state.config, buildingKey, now);
          appendLog(state.economy, result.message, now);
          shouldPersist = true;
        } else if (action === 'allocate-stat') {
          const statKey = target.getAttribute('data-stat');
          if (allocateStatPoint(state.economy, statKey, now)) shouldPersist = true;
        } else if (action === 'respec') {
          if (runFreeRespec(state.economy, now)) shouldPersist = true;
        } else if (action === 'arm-harvest') {
          const profile = getActiveProfile(state.economy, now);
          const daily = ensureProfileDailyState(profile, now);
          if (!daily.harvestSurgeUsed) {
            daily.harvestSurgeArmed = !daily.harvestSurgeArmed;
            appendLog(
              state.economy,
              daily.harvestSurgeArmed ? 'Harvest Surge armed for next Farm collect.' : 'Harvest Surge disarmed.',
              now,
            );
            shouldPersist = true;
          }
        } else if (action === 'time-shift') {
          const hours = toSafeNumber(target.getAttribute('data-hours'), 0);
          state.economy.dev.timeOffsetMs += Math.round(hours * 60 * 60 * 1000);
          appendLog(state.economy, `Simulation advanced by ${hours}h.`, getCurrentNow(state.economy));
          shouldPersist = true;
        } else if (action === 'time-reset') {
          state.economy.dev.timeOffsetMs = 0;
          appendLog(state.economy, 'Simulation clock synced to real time.', getCurrentNow(state.economy));
          shouldPersist = true;
        } else if (action === 'resolve-jobs') {
          const changed = resolveFinishedJobs(state.economy, state.config, now);
          appendLog(state.economy, changed ? 'Pending jobs resolved.' : 'No jobs ready yet.', now);
          shouldPersist = true;
        } else if (action === 'reset-economy') {
          state.economy = sanitizeEconomyV01State(null, state.config, BUILDINGS_LAB_DEFAULT_CHARACTER, Date.now());
          state.economy.username = BUILDINGS_LAB_USERNAME;
          appendLog(state.economy, 'Economy save reset to default v0.1 state.', getCurrentNow(state.economy));
          shouldPersist = true;
        } else if (action === 'reset-inventory') {
          resetCollectedInventory(state.economy, state.config, now);
          shouldPersist = true;
        }

        if (shouldPersist) state.persist();
        renderLab(state);
        syncJobTooltipFromPointer();
      };

      buildingsLabRoot.addEventListener('click', handleLabActionClick);
      if (buildingsSideStats) buildingsSideStats.addEventListener('click', handleLabActionClick);

      buildingsLabRoot.addEventListener('change', (event) => {
        const target = event.target;
        if (!(target instanceof HTMLSelectElement)) return;
        if (target.getAttribute('data-action') !== 'set-character') return;
        const nextCharacter = normalizeVisibleCharacterKey(target.value, BUILDINGS_LAB_DEFAULT_CHARACTER) || BUILDINGS_LAB_DEFAULT_CHARACTER;
        state.economy.activeCharacter = nextCharacter;
        ensureCharacterProfile(state.economy, nextCharacter, getCurrentNow(state.economy));
        appendLog(state.economy, `Active character switched to ${nextCharacter.toUpperCase()}.`, getCurrentNow(state.economy));
        state.persist();
        renderLab(state);
        syncJobTooltipFromPointer();
      });
    ...
    initBuildingsLab().catch((error) => {
      console.error('Failed to initialize buildings lab', error);
      buildingsLabRoot.innerHTML = '<div class="muted">Failed to initialize buildings simulator.</div>';
    });
  }
    ...
    if (bioViewPortfolio) {
      bioViewPortfolio.addEventListener('click', () => {
        const characterQuery = activeBioCharacter
          ...
          : '';
        window.location.href = `./collect.html${characterQuery}`;
      });
    }
