...
(async()=>{
 ...
 s.contracts.offers[2].customer='breakfast';s.contracts.offers[2].name='Breakfast regulars';
 ...
 try{
  ...
  assert(await page.getByText('1/3 visits → +20% roastery customers').count());
  ...
  for(const [slot,id] of [[0,'farm'],[2,'roastery']]){
   ...
   for(const level of [3,6]){
    ...
    assert((await art.getAttribute('src')).endsWith(id+'-level-'+level+'.png'));
  ...
  console.log(JSON.stringify({result:'passed',decisions:calls.length,buildingArtVariants:4,screenshots:5}));
