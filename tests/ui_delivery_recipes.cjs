...
(async()=>{
 ...
 try{
  ...
  for(const [width,height] of [[1366,768],[1366,650],[1728,694],[1920,1080],[1024,768],[768,768],[390,844],[844,390]]){
   ...
   for(let orderPage=0;orderPage<3;orderPage++){
    ...
    for(const slot of cards){
     ...
     assert.equal(await card.locator('.game-order-purpose,.game-order-progress,.game-order-eta').count(),0,'Extra order copy removed');
  ...
  assert.equal(await page.getByText('Materials reduce future construction costs',{exact:true}).count(),0);
  assert.equal(await page.getByText('Saved delivery · original reward preserved; your next card shows town projects',{exact:true}).count(),0);
  await checkBounds('Legacy offer fallback');assert.deepEqual(errors,[]);
  console.log('Passed: named recipes, concise cards, three reward channels, five goods, eight viewports and legacy offers.');
