...
(async()=>{
 ...
 try{
  ...
  let reject=false,hold=false,release,serial=0,replacements=0,nextRarity='standard',nextCanFulfill=true;
  ...
  async function skip(slot=0){await showOrder(slot);await page.locator('[data-order-slot="'+slot+'"] [data-econ-action^="replace:"]').click();await page.waitForFunction(()=>!document.querySelector('[aria-busy="true"]'));}
  async function save(){await showOrder(0);await page.locator('[data-order-slot="0"] .game-order-commit').click();await page.waitForFunction(()=>!document.querySelector('[aria-busy="true"]'));}
  ...
  release();await page.waitForFunction(()=>!document.querySelector('[aria-busy="true"]'));
  ...
  assert.equal(await page.locator('.game-order-reaction').count(),0);
  release();await page.locator('[data-saved-order="0"]').waitFor();
  ...
  await page.waitForFunction(()=>{const image=document.querySelector('[data-saved-order="0"] img');return image&&image.complete&&image.naturalWidth>0;});
  ...
  // Releasing goods, rejected saves, and ordinary saves do not celebrate.
  ...
  state.contracts.offers[0].rarity='standard';
  await page.evaluate(()=>window.YomamaEcon.refresh());
  await save();assert.equal(state.contracts.offers[0].committed,true);
  assert.equal(await page.locator('.game-order-reaction').count(),0);
  ...
  // Saving immediately replaces the crying face with a single reduced-motion dollar.
  ...
  assert.deepEqual(errors,[]);console.log('Passed: readiness, queued jackpot spam, accepted/pending/rejected jackpot saves, release, ordinary saves, rapid clicks, expiry, delivery, mobile, reduced motion, reload.');
