/* The LOADING screen between signing in and the first build page: it shows
   once, it holds the page until the art is in the browser, and it lets go.
...
(async()=>{
 ...
 try{
  ...
  assert.ok(seen.some(v=>v>0),'the bar moved, saw '+seen.join(','));
  for(let i=1;i<seen.length;i++) assert.ok(seen[i]>=seen[i-1],'the bar never goes backwards: '+seen.join(','));

  // It lets go, and what it was waiting for really is loaded by then: every
  // sprite the build page asked for has decoded, so nothing pops in after.
  await gone(page);
  ...
  // Signing in again shows it again, even in a tab that has already had one.
  ...
  await page.fill('#join-code',code);await page.fill('#join-name','LOAD TESTER');await page.fill('#join-pin','4321');
  await Promise.all([page.waitForURL(/buildings\.html/,{timeout:15000}),page.click('#join-btn')]);
  assert.equal(await page.locator('#yomama-loading').count(),1,'a new sign-in gets a new screen');
  await gone(page);

  // Nobody is ever stuck behind it: with every image request failing it still
  // gives up inside its own ceiling and hands the page over.
