/* LOG OUT, idle sign-out and cross-tab sign-out in a real browser, against a
   running server and an open class. Signs a throwaway student in through the
   real join form; no purchases.
...
(async()=>{
 ...
 try{
  ...
  await signIn(page);
  // Every page with the header has a live LOG OUT.
  ...
  // Signed out, the game page shows nobody's base.
  await page.goto(base+'/buildings.html');
  assert.equal((await page.locator('button[data-account]').first().textContent()).trim(),'SIGN IN');
  await page.waitForSelector('text=JOIN A CLASS',{timeout:15000});
